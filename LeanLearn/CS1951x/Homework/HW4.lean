...
def xs_list : List Nat := [1, 2, 3]
def f : Nat → Nat := fun x => x * x

/- We want to calculate -/

#eval List.map f xs_list -- [1, 4, 9]
...
theorem dotAndSwap (P Q R : Prop) (hpq : P ∧ Q ∧ R) : Q ∧ P :=
  ⟨hpq.right.left,hpq.left⟩
...
def enq : SimpleQueue α → α → SimpleQueue α  := flip (·::·)

def deq : SimpleQueue α → Option (α × SimpleQueue α)
| []      => none
| (x::xs) => match deq xs with
  | none        => some (x,xs)
...
#eval @SimpleQueue.emp Nat
#eval (((SimpleQueue.emp.enq 1).enq 3).enq 4).deq
...
-- #eval ((SimpleQueue.emp.enq 1).enq 2)
-- #eval (((((EfficientQueue.emp).enq 0).enq 1).enq 2).deq.get!.snd).toList
...
#check @List.append_assoc Nat

theorem enq_equiv {α : Type} :
  ∀ (sq : SimpleQueue α) (eq : EfficientQueue α) (x : α),
  Equiv sq eq → Equiv (sq.enq x) (eq.enq x) := by
  intros sq eq x heq
  have h : sq.toList = eq.toList := by sorry
  sorry
  done
